...
void TrGEMEventAction::EndOfEventAction(const G4Event* event)
	{
  	...
  	if (event_id % 10000 == 0)
 		...
 		//G4cout<<"primary vertex.......................................................= "<<event->GetPrimaryVertex(0)->GetPosition()<<G4endl;
      			...
      			if (numOfDetectHits > 0)
      				{
      					for (i = 0; i < numOfDetectHits; i++)
      					{
           				...
           				TrGEMAnalysis::GetInstance()->SetValuesForInteractingEvents(event_id, numOfDetectHits, i, aHit);
           				TrGEMAnalysis::GetInstance()->EndOfEvent() ;
           			}		
           		}
      			else	
      				{
      					TrGEMAnalysis::GetInstance()->SetValuesForNonInteractingEvents(event_id, numOfDetectHits);
      					TrGEMAnalysis::GetInstance()->EndOfEvent() ;
        			}
        	}
      }
