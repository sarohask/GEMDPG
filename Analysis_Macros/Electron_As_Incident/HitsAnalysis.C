...
void HitsAnalysis::Loop()
	{
   	...
   	f = new TFile("HitsAnalysis_e-_5deg.root", "RECREATE");
  	...
  	1.1220185e-5, 1.2589254e-5, 1.4125375e-5, 1.5848932e-5, 1.7782794e-5, 1.9952623e-5, 2.238721e-5, 2.5118864e-5, 2.818383e-5, 3.1622777e-5, 3.548134e-5, 
  	3.981072e-5, 4.466836e-5, 5.011872e-5, 5.623413e-5, 6.309573e-5, 7.079458e-5, 7.943282e-5, 8.91251e-5, 1.0e-4,
   	...
   	630.9573, 707.9458, 794.3282, 891.251, 1000.000, 1122.0185
   											};
   	...
   	TH1D* h_PrimaryEnergy = new TH1D("h_PrimaryEnergy"," Primary Energy of Electrons ", 240, binEdge);
    h_PrimaryEnergy->GetXaxis()->SetTitle("Energy (MeV)");
    h_PrimaryEnergy->GetYaxis()->SetTitle("Number of Events");
   	    
   	TH1D* h_ElectronSensitivity = new TH1D("h_ElectronSensitivity"," Electron Sensitivity vs Kinetic Energy", 240, binEdge);
   	h_ElectronSensitivity->GetXaxis()->SetTitle("Energy (MeV)");
   	h_ElectronSensitivity->GetYaxis()->SetTitle("Neutron Sensitivity");
   		
   	TH1I* h_NumberInteraction = new TH1I("h_NumberInteraction"," Number of Interactions of Primary Electron ", 40, 0, 40);
    h_NumberInteraction->GetXaxis()->SetTitle("Number of Interaction");
    h_NumberInteraction->GetYaxis()->SetTitle("Number of Events");
   	
   	TH1D* h_InteractionLength = new TH1D("h_InteractionLength"," Length Traversed within the Detector by Primary Electron ", 800, 0, 400);
    ...
    TH2D* h_LengthInterNumber = new TH2D("h_LengthInterNumber"," Number of Interaction vs Length Traversed by Primary Electron ", 40, 0, 40, 1000, 0, 200);
   	h_LengthInterNumber->GetXaxis()->SetTitle("Number of Interactions");
   	h_LengthInterNumber->GetYaxis()->SetTitle("Traversed Length (mm)");	
    
    TH1I* h_NumberElectron = new TH1I("h_NumberNeutron"," Number of Secondary Electrons Produced per Event ", 200, 0, 200);
    ...
    h_NumberO16->GetYaxis()->SetTitle("Number of Events");
   	...
   	int totNum = 0 ;
   	int event = 0 ;
   	int nPositron = 0 ; 	int nElectron = 0 ; 	int nGamma = 0 ;		int nProton = 0 ;		int nPosPions = 0;		int nNegPions = 0;
   	int nElectronGap1Gas = 0 ;		int nElectronGap2Gas = 0 ;		int nElectronGap3Gas = 0 ;		int nElectronGap4Gas = 0 ;
   	
   	int nDeuteron = 0 ;		int nO16 = 0 ;		int nNeutron = 0 ;
   	bool electronSensitive = false ;
   	...
   	for (Long64_t jentry=0; jentry<nentries;jentry++) 
      	...
      	if (event != event_id)  
      		{      			
      			...
      			//cout<<"..nElectron..= "<<nElectron<<"..GasGap1..= "<<nElectronGap1Gas<<"..GasGap2..= "<<nElectronGap2Gas<<"..GasGap3..= "
      			//<<nElectronGap3Gas<<"..GasGap4..= "<<nElectronGap4Gas<<endl;
      			cout<<event<<"...total gamma ... = "<<nGamma<<"..total electron.= "<<nElectron<<"..protons.. = "<<nProton<<endl;
      			if (electronSensitive == true)
      				{
      					h_ElectronSensitivity->Fill(primEnergy);
      				}
      	...
      	if (event == event_id)
      		{
      			...
      			//cout<<event_id<<"..PrimeEnergy............................................"<<primEnergy<<endl;
      			if (Charge != 0 && TrackID != 1 && *VolumeName != "FakeBottom" && *VolumeName != "FakeTop")																		
      				{ 
      					electronSensitive = true ;		
      				}      			
    ...
    h_NumberO16->Fill(nO16);
    h_InteractionLength->Fill(interLength);
   	h_ElectronSensitivity->Divide(h_PrimaryEnergy);
   	
   	// Write Histo
   	h_ElectronSensitivity->Write();
